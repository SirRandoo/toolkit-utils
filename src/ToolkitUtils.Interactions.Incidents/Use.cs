...
{
    private static readonly Logger Logger = ToolkitLogManager.GetLogger<Use>();
    ...
    public override bool CanHappen(string msg, Viewer viewer)
    {
        ...
        if (!worker.TryGetNextAsInt(out _amount, 1, viewer.GetMaximumPurchaseAmount(_buyableItem.Cost))) _amount = 1;
        ...
        {
            if (!h.IsUsable(item.Thing.Thing)) continue;
    ...
    public override void Execute()
    {
        ...
        try
        {
            for (var _ = 0; _ < _amount; _++) _handler.Use(_pawn, _buyableItem.Thing);
        }
        catch (Exception e)
        {
