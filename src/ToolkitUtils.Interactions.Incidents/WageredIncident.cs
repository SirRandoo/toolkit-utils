...
{
    private static readonly Dictionary<string, IWageredIncidentData> Data = new()
    {
        {
    ...
    {
        if (!Data.TryGetValue(storeIncident.defName, out _data)) return false;

        if (!_data.UseStoryteller)
        {
            string rawPoints = CommandFilter.Parse(msg).Skip(2).FirstOrDefault();

            if (rawPoints.NullOrEmpty() || !VariablesHelpers.PointsWagerIsValid(rawPoints, viewer, ref _wager, ref storeIncident)) return false;
        }

        Map map = Find.RandomPlayerHomeMap;
        _worker = Activator.CreateInstance(_data.WorkerClass) as IncidentWorker;

        if (_worker == null || map == null) return false;

        _params = StorytellerUtility.DefaultParmsNow(_data.ResolveCategory(_worker, storeIncident), map);

        if (!_data.UseStoryteller) _params.points = IncidentHelper_PointsHelper.RollProportionalGamePoints(storeIncident, _wager, _params.points);
    ...
    public override void Execute()
    {
        if (_worker.TryExecute(_params))
        {
            ...
            MessageHelper.ReplyToUser(
                Viewer.username,
                _data.UseStoryteller ? "TKUtils.Wagered.Storyteller".Translate(name, points) : "TKUtils.Wagered.Complete".Translate(name, _wager.ToString("N0"), points)
            );

            return;
        }

        MessageHelper.ReplyToUser(Viewer.username, "TKUtils.FailedParms".Translate(storeIncident.label ?? storeIncident.abbreviation));
    }
}
