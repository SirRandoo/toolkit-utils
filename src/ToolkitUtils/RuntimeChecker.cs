...
namespace ToolkitUtils
{
    ...
    {
        private static readonly IRimLogger Logger = new RimLogger("ToolkitUtils.RuntimeChecker");
        ...
        private static void TryLanceTicker()
        {
            ...
            {
                Logger.Warn($"Could not lance Toolkit's ticker; it was an unexpected value of {tickerField.GetValue(Toolkit.Mod).GetType().FullDescription()}");
            ...
            {
                Logger.Error("Could not abort Toolkit's ticker thread", e);
            }
        ...
        private static void ValidateToolkitTicker()
        {
            ...
            {
                Logger.Error("Could not instantiate Toolkit's ticker. Restart your game, or re-enabling Utils' command router.", e);
            }
        }
    }
}
