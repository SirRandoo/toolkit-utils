...
namespace ToolkitUtils
{
    ...
    {
        private static readonly IRimLogger Logger = new RimThreadedLogger("TKU.CompatRegistry");
        ...
        public static bool IsHealable(Hediff hediff)
        {
            foreach (IHealHandler handler in HealHandlers)
            {
                ...
                {
                    Logger.Info($"The handler {handler.GetType().FullDescription()} requested that the hediff {hediff.def.defName} not be healed.");
        ...
        public static bool IsHealable(BodyPartRecord record)
        {
            foreach (IHealHandler handler in HealHandlers)
            {
                ...
                {
                    Logger.Info($"The handler {handler.GetType().FullDescription()} requested that the body part {record.def.defName} not be healed.");
