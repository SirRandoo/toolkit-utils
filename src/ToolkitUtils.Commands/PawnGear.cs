...
namespace ToolkitUtils.Commands
{
    ...
    public class PawnGear : CommandBase
    {
        public override void RunCommand([NotNull] ITwitchMessage twitchMessage)
        {
            ...
            {
                twitchMessage.Reply("TKUtils.NoPawn".TranslateSimple().WithHeader("TabGear".TranslateSimple()));
        ...
        private static string GetPawnGear(Pawn pawn)
        {
            ...
            parts.Add($"{"Apparel".TranslateSimple()}: {apparel.Select(item => RichTextHelper.StripTags(item.LabelCap)).SectionJoin()}");
        ...
        private static void GetTemperatureValues(Thing pawn, [NotNull] ICollection<string> parts)
        {
            ...
            parts.Add($"{ResponseHelper.TemperatureGlyph.AltText($"{"ComfyTemperatureRange".TranslateSimple()} ")}{tempMin}~{tempMax}");
        }
        ...
        private static void GetArmorValues([NotNull] Pawn pawn, ICollection<string> parts)
        {
            ...
            if (sharp > 0)
            {
                stats.Add($"{ResponseHelper.DaggerGlyph.AltText($"{"ArmorSharp".TranslateSimple()} ")}{sharp.ToStringPercent()}");
            }

            if (blunt > 0)
            {
                stats.Add($"{ResponseHelper.PanGlyph.AltText($"{"ArmorBlunt".TranslateSimple()} ")}{blunt.ToStringPercent()}");
            }

            if (heat > 0)
            {
                stats.Add($"{ResponseHelper.FireGlyph.AltText($"{"ArmorHeat".TranslateSimple()} ")}{heat.ToStringPercent()}");
            }
