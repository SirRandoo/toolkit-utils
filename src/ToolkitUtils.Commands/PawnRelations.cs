...
namespace ToolkitUtils.Commands
{
    ...
    public class PawnRelations : CommandBase
    {
        ...
        private static void ShowRelationshipOverview([NotNull] ITwitchMessage twitchMessage, [NotNull] GameComponentPawns component, Pawn pawn)
        {
            ...
            twitchMessage.Reply(container.Count <= 0 ? "TKUtils.PawnRelations.None".TranslateSimple() : container.SectionJoin());
        }
        ...
        private static string GetSocialString(Pawn pawn, Pawn otherPawn, int opinion, bool overrideSettings = false)
        {
            ...
            if (opinion < -20)
            {
                return "Rival".TranslateSimple();
            }

            return opinion > 20 ? "Friend".TranslateSimple() : "Acquaintance".TranslateSimple();
        }
    }
}
