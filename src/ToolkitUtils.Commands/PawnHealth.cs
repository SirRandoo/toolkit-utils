...
namespace ToolkitUtils.Commands
{
    ...
    public class PawnHealth : CommandBase
    {
        public override void RunCommand([NotNull] ITwitchMessage twitchMessage)
        {
            ...
            {
                twitchMessage.Reply("TKUtils.NoPawn".TranslateSimple().WithHeader("TabHealth".TranslateSimple()));
            ...
            twitchMessage.Reply((capacity == null ? HealthReport(pawn!) : HealthCapacityReport(pawn!, capacity)).WithHeader("TabHealth".TranslateSimple()));
        }
        ...
        private static string HealthCapacityReport([NotNull] Pawn pawn, [NotNull] PawnCapacityDef capacity)
        {
            ...
            {
                return "TKUtils.PawnHealth.IncapableOfCapacity".Translate(capacity.GetLabelFor(pawn));
            }
            ...
            var segments = new List<string>
            {
                ...
                impactors.Any()
                    ? ResponseHelper.JoinPair("TKUtils.PawnHealth.AffectedBy".TranslateSimple(), GetImpactorsForPawn(pawn, impactors).SectionJoin())
                    : "NoHealthConditions".TranslateSimple().CapitalizeFirst()
            };
        ...
        private static string HealthReport([NotNull] Pawn pawn)
        {
            var segments = new List<string>
            {
                ResponseHelper.JoinPair("TKUtils.PawnHealth.OverallHealth".TranslateSimple(), pawn.health.summaryHealth.SummaryHealthPercent.ToStringPercent())
            };
            ...
            if (pawn.health.hediffSet.BleedRateTotal > 0.01f)
            {
                ...
                segments.Add(
                    ticks >= 60000
                        ? ResponseHelper.BleedingSafeGlyphs.AltText("WontBleedOutSoon".TranslateSimple().CapitalizeFirst())
            ...
            {
                segments.Add("TKUtils.Responses.UnsupportedRace".Translate(pawn.kindDef.race.defName));
            }
            ...
            segments.Add(ResponseHelper.JoinPair("TKUtils.PawnHealth.QueuedSurgeries".TranslateSimple(), queued.SectionJoin()));
