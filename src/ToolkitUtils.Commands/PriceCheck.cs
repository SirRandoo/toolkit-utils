...
namespace ToolkitUtils.Commands
{
    ...
    public class PriceCheck : CommandBase
    {
        ...
        private void PerformAnimalLookup(string query, int quantity)
        {
            ...
            {
                MessageHelper.ReplyToUser(_invoker, "TKUtils.Overflowed".TranslateSimple());

                return;
            }

            NotifyLookupComplete("TKUtils.Price.Limited".Translate(kindDef.defName.CapitalizeFirst(), result.ToString("N0")));
        }
        ...
        private void PerformEventLookup(string query)
        {
            ...
            switch (eventType)
            {
                case EventTypes.Default:
                    NotifyLookupComplete("TKUtils.Price.Limited".Translate(result.abbreviation.CapitalizeFirst(), result.cost.ToString("N0")));

                    return;
                case EventTypes.Item:
                case EventTypes.PawnKind:
                case EventTypes.Trait:
                    NotifyLookupComplete("TKUtils.Price.Overridden".Translate(eventType.ToString()));

                    return;
                case EventTypes.Misc:
                    NotifyLookupComplete("TKUtils.Price.External".TranslateSimple());
                ...
                case EventTypes.Variable:
                    NotifyLookupComplete(
                        new string[]
                        {
                            "TKUtils.Price.Variables".Translate(result.abbreviation.CapitalizeFirst(), result.cost.ToString("N0")),
                            "TKUtils.Price.External".TranslateSimple()
                        }.GroupedJoin()
                    );
        ...
        private void PerformItemLookup([NotNull] string query, int quantity)
        {
            ...
            NotifyLookupComplete("TKUtils.Price.Limited".Translate(item.AsString().CapitalizeFirst(), total.ToString("N0")));
        }
        ...
        private void PerformKindLookup(string query)
        {
            ...
            NotifyLookupComplete("TKUtils.Price.Limited".Translate(result!.Name.ToToolkit().CapitalizeFirst(), result.Cost.ToString("N0")));
        }
        ...
        private void PerformTraitLookup(string query)
        {
            ...
            {
                parts.Add("TKUtils.Price.AddTrait".Translate(result.CostToAdd.ToString("N0")));
            }

            if (result.CanRemove && CanRemoveTrait)
            {
                parts.Add("TKUtils.Price.RemoveTrait".Translate(result.CostToRemove.ToString("N0")));
            }
