...
namespace ToolkitUtils.Commands
{
    ...
    public class Balance : CommandBase
    {
        public override void RunCommand([NotNull] ITwitchMessage twitchMessage)
        {
            ...
            {
                container.Add(ResponseHelper.JoinPair("TKUtils.Balance.Coins".TranslateSimple().CapitalizeFirst(), coins));
                container.Add(ResponseHelper.JoinPair("TKUtils.Balance.Karma".TranslateSimple().CapitalizeFirst(), karma));
            }
            ...
            if (ToolkitSettings.EarningCoins && TkSettings.ShowCoinRate)
            {
                ...
                container.Add(
                    (income > 0 ? $"{ResponseHelper.IncomeGlyph} +{income:N0}" : $"{ResponseHelper.DebtGlyph} {income:N0}").AltText(
                        "TKUtils.Balance.Rate".Translate(CalculateCoinAward(viewer).ToString("N0"), ToolkitSettings.CoinInterval.ToString("N0"))
                    )
                );
            }
