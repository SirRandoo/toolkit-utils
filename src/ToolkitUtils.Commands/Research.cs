...
namespace ToolkitUtils.Commands
{
    ...
    public class Research : CommandBase
    {
        public override void RunCommand([NotNull] ITwitchMessage twitchMessage)
        {
            ...
            {
                twitchMessage.Reply(
                    (!query.NullOrEmpty() ? (string) "TKUtils.Research.InvalidQuery".Translate(query) : "TKUtils.Research.None".TranslateSimple()).WithHeader(
                        "Research".TranslateSimple()
                    )
                );
            ...
            if (project.prerequisites != null && !project.PrerequisitesCompleted)
            {
                ...
                segments.Add(ResponseHelper.JoinPair("ResearchPrerequisites".TranslateSimple(), container.SectionJoin()));
            }
