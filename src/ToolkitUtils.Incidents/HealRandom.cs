    ...
    public class HealRandom : IncidentVariablesBase
    {
        ...
        public override bool CanHappen(string msg, Viewer viewer)
        {
            List<Pawn> pawns = Find.ColonistBar.GetColonistsInOrder()
               .Where(p => !p.Dead)
               .Where(
                    pawn => !global::ToolkitUtils.IncidentSettings.HealRandom.FairFights
        ...
        private void NotifySuccess(string affected)
        {
            ...
            string descriptionTranslated = description.Translate(_target.LabelShort.CapitalizeFirst(), affected);
            MessageHelper.SendConfirmation(Viewer.username, descriptionTranslated);

            Current.Game.letterStack.ReceiveLetter("TKUtils.HealLetter.Title".TranslateSimple(), descriptionTranslated, LetterDefOf.PositiveEvent, _target);
        }
    }
}
