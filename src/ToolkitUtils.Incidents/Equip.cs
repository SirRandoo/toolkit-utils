...
namespace ToolkitUtils.Incidents
{
    ...
    {
        private static readonly IRimLogger Logger = new RimThreadedLogger("ToolkitUtils.Incidents.Equip");
        ...
        public override void Execute()
        {
            ...
            {
                Logger.Warn("Tried to equip a null weapon.");
            ...
            {
                Logger.Warn($"Could not make room for {Viewer.username}'s new weapon.");
            }
        ...
        private void NotifyComplete([NotNull] Thing thing, bool spawned = false)
        {
            MessageHelper.SendConfirmation(
                Viewer.username,
                (spawned ? "TKUtils.Equip.Spawned" : "TKUtils.Equip.Complete").Translate(thing.LabelCap ?? thing.def.defName, _cost.ToString("N0"))
            );

            Find.LetterStack.ReceiveLetter(
                "TKUtils.EquipLetter.Title".TranslateSimple(),
                (spawned ? "TKUtils.EquipLetter.SpawnedDescription" : "TKUtils.EquipLetter.Description").Translate(
                    Viewer.username,
