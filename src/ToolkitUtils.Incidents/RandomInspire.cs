        ...
        public override void Execute()
        {
            foreach (Pawn pawn in Find.ColonistBar.GetColonistsInOrder().InRandomOrder())
            {
                ...
                MessageHelper.SendConfirmation(
                    Viewer.username,
                    "TKUtils.RandomInspire.Complete".Translate(pawn.LabelShort?.CapitalizeFirst() ?? pawn.LabelCap, def.label)
                );
            ...
            MessageHelper.ReplyToUser(Viewer.username, "TKUtils.RandomInspire.None".TranslateSimple());
        }
    }
}
