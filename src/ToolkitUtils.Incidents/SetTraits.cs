...
namespace ToolkitUtils.Incidents
{
    public class SetTraits : IncidentVariablesBase
    {
        ...
        public override bool CanHappen(string msg, Viewer viewer)
        {
            ...
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.LimitReached".Translate(AddTraitSettings.maxTraits));
        ...
        private bool TryProcessTraits([NotNull] Pawn subject, [NotNull] IEnumerable<TraitItem> traits, out List<TraitEvent> traitEvents)
        {
                ...
                traitItems.Where(t => !subject.story.traits.HasTrait(t.TraitDef))
                   .Select(
                        t =>
                        {
                            if (!t.CanAdd)
                            {
                                return new TraitEvent { Type = EventType.Noop, Error = "TKUtils.Trait.Disabled".Translate(t.Name) };
                            }
            ...
            foreach (TraitEvent traitEvent in final.Where(f => f.Type == EventType.Add || f.Type == EventType.Noop))
            {
                ...
                {
                    traitEvent.Error = "TKUtils.Trait.Conflict".Translate(traitEvent.Item.Name, conflictsWith.Item.Name);
        ...
        public override void Execute()
        {
            ...
            MessageHelper.SendConfirmation(Viewer.username, "TKUtils.SetTraits.Complete".Translate(traitString));

            Find.LetterStack.ReceiveLetter(
                "TKUtils.TraitLetter.Title".TranslateSimple(),
                "TKUtils.TraitLetter.SetDescription".Translate(Viewer.username, traitString),
        ...
        private bool IsTraitRemovable(Pawn pawn, [NotNull] TraitItem trait, out string error)
        {
            if (!TraitHelper.IsRemovalAllowedByGenes(pawn, trait.TraitDef, trait.Degree))
            {
                error = "TKUtils.RemoveTrait.GeneLocked".Translate(trait.Name);
            ...
            {
                error = "TKUtils.RemoveTrait.Kind".Translate(_pawn.kindDef.race.LabelCap, trait.Name);
            ...
            {
                error = "TKUtils.RemoveTrait.RationalRomance".Translate(trait.Name.CapitalizeFirst());
            ...
            {
                error = "TKUtils.RemoveTrait.Class".Translate(trait.Name);
            ...
            if (!trait.CanRemove)
            {
                error = "TKUtils.RemoveTrait.Disabled".Translate(trait.Name);
        ...
        private bool IsTraitAddable([NotNull] TraitItem trait, out string error)
        {
            ...
            {
                error = "TKUtils.Trait.RestrictedByBackstory".Translate(backstory.identifier, trait.Name);
            ...
            {
                error = "TKUtils.Trait.GeneSuppressed".Translate(trait.Name);
            ...
            {
                error = "TKUtils.Trait.RestrictedByKind".Translate(_pawn.kindDef.race.LabelCap, trait.Name);
            ...
            if (!trait.CanAdd)
            {
                error = "TKUtils.Trait.Disabled".Translate(trait.Name);
