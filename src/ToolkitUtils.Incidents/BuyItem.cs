...
namespace ToolkitUtils.Incidents
{
    ...
    {
        private static readonly IRimLogger Logger = new RimThreadedLogger("ToolkitUtils.Incidents.BuyItem");
        ...
        public override bool CanHappen(string msg, [NotNull] Viewer viewer)
        {
            ...
            if (BuyItemSettings.mustResearchFirst && projects.Count > 0)
            {
                MessageHelper.ReplyToUser(
                    viewer.username,
                    "TKUtils.ResearchRequired".Translate(product.Thing.Thing.LabelCap.RawText, projects.Select(p => p.LabelCap.RawText).SectionJoin())
                );
            ...
            if (_purchaseRequest.Price < ToolkitSettings.MinimumPurchasePrice)
            {
                MessageHelper.ReplyToUser(
                    viewer.username,
                    "TKUtils.Item.MinimumViolation".Translate(_purchaseRequest.Price.ToString("N0"), ToolkitSettings.MinimumPurchasePrice.ToString("N0"))
                );
            ...
            MessageHelper.ReplyToUser(
                viewer.username,
                "TKUtils.InsufficientBalance".Translate(_purchaseRequest.Price.ToString("N0"), viewer.GetViewerCoins().ToString("N0"))
            );
        ...
        public override void Execute()
        {
            ...
            {
                Logger.Warn($"Buy item failed to execute with error message: {e.Message}");
            }
        }
    }
    ...
    {
        private static readonly IRimLogger Logger = new RimThreadedLogger("TKU.Internal.PurchaseRequest");
        ...
        private void SpawnAnimal()
        {
            ...
            {
                Logger.Warn("Tried to spawn a humanlike -- Humanlikes should be spawned via !buy pawn");
            ...
            {
                Label = "TKUtils.ItemLetter.Animal".Translate(animal),
        ...
        private void SpawnItem()
        {
            ...
            Find.LetterStack.ReceiveLetter(
                ...
                "TKUtils.ItemLetter.ItemDescription".Translate(Quantity.ToString("N0"), Proxy.AsString(Quantity > 1), Purchaser.username),
        ...
        private void NotifyAnimalPurchaseComplete()
        {
            if (TkSettings.BuyItemBalance)
            {
                MessageHelper.SendConfirmation(
                    Purchaser.username,
                    "TKUtils.Item.Complete".Translate(
            ...
            else
            {
                MessageHelper.SendConfirmation(
                    Purchaser.username,
                    "TKUtils.Item.CompleteMinimal".Translate(
                        Quantity.ToString("N0"),
                        Quantity > 1 ? Proxy.Thing.Thing.label.Pluralize() : Proxy.Thing.Thing.label,
                        Price.ToString("N0")
                    )
            ...
            {
                Find.LetterStack.ReceiveLetter(
                    "TKUtils.ItemLetter.Animal".Translate(
                        Quantity > 1 ? Proxy.Thing.Thing.label.CapitalizeFirst().Pluralize() : Proxy.Thing.Thing.label.CapitalizeFirst()
                    ),
                    "LetterFarmAnimalsWanderIn".Translate(Proxy.Thing.Thing.label.Pluralize()),
        ...
        private void NotifyItemPurchaseComplete()
        {
            MessageHelper.SendConfirmation(
                ...
                TkSettings.BuyItemBalance
                    ...
                    : "TKUtils.Item.CompleteMinimal".Translate(Quantity.ToString("N0"), Proxy.AsString(Quantity > 1), Price.ToString("N0"))
            );
        }
    }
}
