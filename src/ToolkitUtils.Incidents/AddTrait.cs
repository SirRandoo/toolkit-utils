...
namespace ToolkitUtils.Incidents
{
    public class AddTrait : IncidentVariablesBase
    {
        ...
        public override bool CanHappen(string msg, [NotNull] Viewer viewer)
        {
            ...
            if (!viewer.CanAfford(_buyableTrait!.CostToAdd))
            {
                MessageHelper.ReplyToUser(
                    viewer.username,
                    "TKUtils.InsufficientBalance".Translate(_buyableTrait.CostToAdd.ToString("N0"), viewer.GetViewerCoins().ToString("N0"))
                );
        ...
        private bool PassesValidationChecks(Viewer viewer)
        {
            ...
            if (conflicting != null)
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.Conflict".Translate(conflicting.LabelCap, _trait.LabelCap));
            ...
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.Duplicate".Translate(duplicateTrait.Label, _trait.Label));
            ...
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.GeneSuppressed".Translate(_buyableTrait.Name));
        ...
        private bool PassesCharacterChecks(Viewer viewer, ArgWorker worker)
        {
            ...
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.RestrictedByBackstory".Translate(backstory.identifier, worker.GetLast()));
            ...
            {
                MessageHelper.ReplyToUser(viewer.username, "TKUtils.Trait.RestrictedByKind".Translate(_pawn.kindDef.race.LabelCap, worker.GetLast()));
        ...
        public override void Execute()
        {
            ...
            MessageHelper.SendConfirmation(Viewer.username, "TKUtils.Trait.Complete".Translate(_trait.Label));

            Current.Game.letterStack.ReceiveLetter(
                "TKUtils.TraitLetter.Title".TranslateSimple(),
                "TKUtils.TraitLetter.AddDescription".Translate(Viewer.username, _trait.LabelCap),
