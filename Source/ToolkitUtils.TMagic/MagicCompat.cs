...
[UsedImplicitly]
public record MagicCompat(string ModId = "Torann.ARimworldOfMagic") : IMagicCompatibilityProvider
{
    ...
    public string GetSkillDescription(string invoker, string query)
    {
        ...
        if (userMight is not { IsMightUser: true } || !TryGetMightDescription(userMight, query.ToToolkit(), out description))
        {
    ...
    private static bool TryGetGlobalMagicDescription(CompAbilityUserMagic userMagic, string query, [NotNullWhen(true)] out string? s)
    {
        ...
        {
            s = userMagic.MagicData.MagicPowerSkill_global_regen.FirstOrDefault()?.desc.Localize();
        ...
        {
            s = userMagic.MagicData.MagicPowerSkill_global_eff.FirstOrDefault()?.desc.Localize();
        ...
        {
            s = userMagic.MagicData.MagicPowerSkill_global_spirit.FirstOrDefault()?.desc.Localize();
    ...
    private static bool TryGetGlobalMightDescription(CompAbilityUserMight userMight, string query, [NotNullWhen(true)] out string? s)
    {
        ...
        {
            s = userMight.MightData.MightPowerSkill_global_refresh.FirstOrDefault()?.desc.Localize();
        ...
        if (query.EqualsIgnoreCase(efficiency) || query.EqualsIgnoreCase("efficiency"))
        {
            s = userMight.MightData.MightPowerSkill_global_seff.FirstOrDefault()?.desc.Localize();
        ...
        {
            s = userMight.MightData.MightPowerSkill_global_strength.FirstOrDefault()?.desc.Localize();
        ...
        {
            s = userMight.MightData.MightPowerSkill_global_endurance.FirstOrDefault()?.desc.Localize();
    ...
    private static bool TryGetMagicSkillDescription(CompAbilityUserMagic userMagic, string query, [NotNullWhen(true)] out string? s)
    {
        ...
        {
            if (!magicPower.learned || magicPower.abilityDef is not TMAbilityDef def)
            {
                continue;
            }
