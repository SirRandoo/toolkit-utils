...
public static class ClassData
{
    ...
    private static Tuple<TraitDef, List<Ability>> PowerPair(TraitDef trait, List<Ability> abilities) => new(trait, abilities);
    ...
    private static List<Ability> GetChaosMagePowers()
    {
        ...
        list.AddRange(
            GetNecromancerPowers()
               .Where(p => !IsAbility(p, TorannMagicDefOf.TM_RaiseUndead) || !IsAbility(p, TorannMagicDefOf.TM_LichForm) || !IsAbility(p, TorannMagicDefOf.TM_DeathBolt))
        );
    ...
    private static Class GenerateClassTree(TraitDef trait)
    {
    ...
    private static IEnumerable<Ability> GetAbilitiesFor(TraitDef trait)
    {
    ...
    private static List<Ability>? GetRawPowersForBaseClass(TraitDef trait) => !BaseClassPowers.TryGetValue(trait, out List<Ability> powers) ? null : powers;

    private static bool IsAbility(IPawnPower ability, Def def) => ability.Name == (def.label ?? def.defName);
}
