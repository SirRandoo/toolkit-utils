...
using SirRandoo.ToolkitUtils.Constraints;
...
namespace SirRandoo.ToolkitUtils.Windows
{
    public class Dialog_PurgeViewers : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            var buttonText = (showingAffected ? "TKUtils.Windows.Purge.Buttons.Confirm" : "TKUtils.Windows.Purge.Buttons.Execute").Translate();
            ...
            if(showingAffected)
            {
                ...
                for(var i = 0; i < affected.Length; i++)
                {
                    ...
                    Widgets.Label(new Rect(contentView.x, contentView.y + (i * 40f), inRect.width - exemptWidth - 20f, 30f), viewer.username);

                    if(Widgets.ButtonText(new Rect(contentView.width - exemptWidth, contentView.y + (i * 40f), exemptWidth, 30f), exemptText))
                    {
            ...
            else
            {
                ...
                for(var i = 0; i < constraints.Count; i++)
                {
                    ...
                    constraint.Draw(new Rect(contentView.x, contentView.y + (i * 40f), contentView.width - exemptWidth - 20f, 30f));
        ...
        public void DrawHeader(Rect region)
        {
            if(showingAffected)
            {
                ...
                var textRegion = new Rect(region.x + backRegion.width + 10f, region.y, region.width - backRegion.width - 10f, region.height);
                ...
                var affected = GetAffectedViewers();
                Widgets.Label(textRegion, "TKUtils.Windows.Purge.Headers.Affected".Translate(affected.Length.Named("COUNT")));
            }
            else
            {
                var width = region.width * 0.25f;

                if(Widgets.ButtonText(new Rect(region.x, region.y, width, region.height), "TKUtils.Windows.Purge.Buttons.Add".Translate()))
                {
                ...
                if(Widgets.ButtonText(new Rect(region.x + width + 10f, region.y, width, region.height), "TKUtils.Windows.Purge.Buttons.ClearConstraints".Translate()))
                {
                    constraints.Clear();
                }
            }
