...
public abstract class ItemWorkerBase<T, TU> where T : TableWorker<TableSettingsItem<TU>> where TU : class, IShopItemBase
{
    ...
    private void AddSelector(ISelectorBase<TU> selector)
    {
        selector.Prepare();
    ...
    private void AddMutator(IMutatorBase<TU> mutator)
    {
        mutator.Prepare();
    ...
    [CanBeNull]
    private static IMutatorBase<TU> CreateMutator(DomainIndexer.MutatorEntry entry) =>
    ...
    [CanBeNull]
    private static ISelectorBase<TU> CreateSelector(DomainIndexer.SelectorEntry entry) =>
