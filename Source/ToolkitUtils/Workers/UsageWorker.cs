...
public class UsageWorker<T> where T : class, IUsageItemBase
{
    ...
    public bool IsOnCooldown(T item) => _records.TryGetValue(item.DefName, out UsageRecord<T> record) && record.IsOnCooldown();
    ...
    public bool IsOnCooldown(T item, string user)
    {
    ...
    public void RecordUsage(T item)
    {
    ...
    public void RecordUsage(T item, string user)
    {
